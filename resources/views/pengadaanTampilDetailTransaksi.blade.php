    ...
    <div class="breadcrumb-holder">
      <div class="container-fluid">
        <ul class="breadcrumb">
          ...
          <li class="breadcrumb-item active">Tampil Detail Pengadaan </li>
        </ul>
      </div>
    </div>
    <section class="forms">
      <div class="container-fluid">
            <!-- Page Header-->
        <form action="{{ url('/pengadaanTampil') }}" >
        ...
        <div class="row">
            <div class="col-lg-12">
              <div class="card">
              <br>
                <h4>&emsp;Tampil Detail Pengadaan</h4>
                <div class="card-body">
                    ...
                    <tbody>
                      <tr>
                        <td>{{ $detail_pengadaan->ta_id }}</td>
                        <td>{{ $detail_pengadaan->sp_nama }}</td>
                        <td>{{ $detail_pengadaan->dpgd_satuan }}</td>
                        <td>{{ $detail_pengadaan->dpgd_jumlah }}</td>
                        <td>{{ $detail_pengadaan->sp_hargaBeli}}</td>
                        <td style="width:100px">
                          <form action="{{ url('/pengadaanInput') }}" >
                            <input name="tambah" type="hidden" value="Tambah">
                            <button class="btn btn-primary" type="submit">Tambah</button>
                          </form>
                           <form action="{{ url('api/detail_pengadaan', [$detail_pengadaan->dpgd_id]) }}" method="post">
                            {{ method_field('DELETE') }}
                            @csrf
                            <input name="delete" type="hidden" value="DELETE">
                            <button onclick=" return confirm('Ingin membatalkan pengadaan ini?')" style="margin-top:-60px; margin-left:100px" class="btn btn-danger" type="submit">Delete</button>
                          </form>
                        </td>
