  ...
  <div class="page">
    ...
    <div class="breadcrumb-holder">
      <div class="container-fluid">
        <ul class="breadcrumb">
          ...
          <li class="breadcrumb-item active">Tampil Pengaadan </li>
          <form action="{{ url('/suratPengadaanInput') }}" >
            <input name="cetak" type="hidden" value="cetak">
          <button class="btn btn-outline-info" type="submit" style="margin-left:250px"> <i class="fa fa-plus-square"></i>Print Surat Pengadaan</button>
        </form>
        </ul>
      </div>
    </div>
    <section class="forms">
      <div class="container-fluid">
          ...
          <div class="col-lg-12">
            <div class="card">
              <div class="card-header d-flex align-items-center">
                <h4>Cari Pengadaan</h4>
              </div>
              <div class="card-body">
                <form action="{{ url()->current() }}">
                  <div class="form-group">
                    <label for="inlineFormInput" class="sr-only">Transaksi Pengadaan ID</label>
                    <input id="ta_id" type="text" placeholder="" name="ta_id" class="mr-3 form-control">
                  </div>
        ...
        <form action="{{ url('/pengadaanInput') }}" >
              ...
              <div class="card-body">
                <div class="table-responsive">
                  <table class="table table-striped table-hover">
                    ...
                    @foreach($transaksi_pengadaans as $transaksi_pengadaan)
                    <tbody>
                      <tr>
                        <td>{{ $transaksi_pengadaan->ta_id }}</td>
                        <td>{{ $transaksi_pengadaan->su_id.'.) '.$transaksi_pengadaan->su_nama }}</td>
                        <td>{{ $transaksi_pengadaan->ta_status }}</td>
                        <td>
                          <form action="{{ url('/pengadaanTampilDetailTransaksi',[$transaksi_pengadaan->ta_id]) }}" >
                        ...
                        <td style="width:100px">
                          ...
                          <form action="{{ url('pengadaanEdit', [$transaksi_pengadaan->ta_id]) }}" method="post">
                          {{ method_field('GET') }} 
                            @csrf
                          ...
                          <form action="{{ url('api/transaksi_pengadaan', [$transaksi_pengadaan->ta_id]) }}" method="post">
                            {{ method_field('DELETE') }}
                            @csrf
                            <input name="delete" type="hidden" value="DELETE">
                            <button onclick=" return confirm('Ingin membatalkan pengadaan ini?')" style="margin-top:-102px; margin-left:132px" class="btn btn-danger" type="submit">Delete</button>
                          </form>
                        </td>
                      </tr>
                    </tbody>
